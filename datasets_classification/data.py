...
from datasets_classification import utils
...
class TSRegressionArchive(BaseData):
    """
    Dataset class for datasets_classification included in:
    ...
    def load_all(self, root_dir, file_list=None, pattern=None):
        """
        Loads datasets_classification from csv files contained in `root_dir` into a dataframe, optionally choosing from `pattern`
        Args:
    ...
    def load_single(self, filepath):
        ...
        if self.config['task'] == 'regression':
            ...
            df, labels = datasets.load_from_tsfile_to_dataframe(filepath, return_separate_X_and_y=True,
                                                                 replace_missing_vals_with='NaN')
            ...
            try:
                data = datasets.load_from_tsfile_to_dataframe(filepath, return_separate_X_and_y=True,
                                                               replace_missing_vals_with='NaN')
        ...
        # sample index (i.e. the same scheme as all datasets_classification in this project)
