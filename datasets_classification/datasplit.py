...
def split_dataset(data_indices, validation_method, n_splits, validation_ratio, test_set_ratio=0,
    ...
    """
    Splits dataset (i.e. the global datasets_classification indices) into a test set and a training/validation set.
    The training/validation set is used to produce `n_splits` different configurations/splits of indices.

    Returns:
        test_indices: numpy array containing the global datasets_classification indices corresponding to the test set
            (empty if test_set_ratio is 0 or None)
        train_indices: iterable of `n_splits` (num. of folds) numpy arrays,
            each array containing the global datasets_classification indices corresponding to a fold's training set
        val_indices: iterable of `n_splits` (num. of folds) numpy arrays,
            each array containing the global datasets_classification indices corresponding to a fold's validation set
    """
...
class DataSplitter(object):
    ...
    def __init__(self, data_indices, data_labels=None):
        ...
        self.data_indices = data_indices  # global datasets_classification indices
        self.data_labels = data_labels  # global raw datasets_classification labels
...
class StratifiedShuffleSplitter(DataSplitter):
    ...
    def split_testset(self, test_ratio, random_state=1337):
        ...
        # return global datasets_classification indices and labels
    ...
    def split_validation(self, n_splits, validation_ratio, random_state=1337):
        ...
        # return global datasets_classification indices per fold
