...
from datasets_classification import dataset
import losses
import encoders
...
class TimeSeriesEncoder():
    ...
    def fit_encoder(self, X, y=None, save_memory=False, verbose=False):
        ...
        varying = bool(numpy.isnan(numpy.sum(X))) # 261,3,1751

        train = torch.from_numpy(X)
        if self.cuda:
            train = train.cuda()

        train_torch_dataset = dataset.Dataset(X)
    ...
    def encode(self, X, batch_size=50):
        ...
        test = dataset.Dataset(X)
        test_generator = torch.utils.data.DataLoader(test, batch_size=batch_size if not varying else 1, shuffle=False)
...
class TimeSeriesCausalCNNEncoder(TimeSeriesEncoder):
    """
    ...
    def __init__(self, compared_length=50, nb_random_samples=10,
        ...
        super(TimeSeriesCausalCNNEncoder, self).__init__(
    ...
    def __create_encoder(self, in_channels, channels, depth, reduced_size,
        ...
        encoder = encoders.causalcnn.CausalCNNEncoder(
...
class TimeSeriesEncoderClassifier(sklearn.base.BaseEstimator,
    ...
    def fit_encoder(self, X, y=None, save_memory=False, verbose=False):
        ...
        while i < self.nb_steps:
            ...
            if self.local_rank != -1:
                train_sampler.set_epoch(i)
            for batch in train_generator: # batch: 10,3,1751
                if self.cuda:
                    if self.local_rank != -1:
                        batch = batch.cuda(self.local_rank)
                    else:
...
class LSTMEncoderClassifier(TimeSeriesEncoderClassifier):
    ...
    def __create_encoder(self, cuda, gpu,local_rank):
        encoder = encoders.lstm.LSTMEncoder()
        encoder.double()
        if cuda:
            if local_rank != -1:
                torch.cuda.set_device(local_rank)
